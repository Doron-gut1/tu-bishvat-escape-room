...
const AreaCard = ({ area, completedTasks, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={`
        p-6 rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105 cursor-pointer
        ${completedTasksInArea === totalTasksInArea ? 'bg-green-100' : 'bg-white'}
      `}
    >
      {/* כותרת ואייקון */}
      <div className="flex items-center mb-4">
        <span className="text-4xl ml-4">{area.icon}</span>
        <div>
          <h3 className="text-xl font-bold text-green-800">{area.name}</h3>
      ...
      {area.biblicalConnection && (
        <div className="text-sm text-gray-600 border-r-2 border-green-500 pr-2 mt-4">
          {area.biblicalConnection}
        </div>
      )}
